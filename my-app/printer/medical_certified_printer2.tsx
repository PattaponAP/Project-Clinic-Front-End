...
export default function doctor_note_printer(
    ...
    doc.setFontSize(26);
    doc.text('ใบรับรองแพทย์', 105, 15, { align: 'center' });
    ...
    doc.text(`4. ประวัติอื่นที่สำคัญ`, 20, 99);
    doc.setFontSize(14);
    doc.setFont("THSarabunNew", "italic");
    doc.text(`* ในกรณีเด็กที่ไม่สามารถรับรองตนเองได้ ให้ผู้ปกคลองลงนามรับรองแทนได้`, 187, 118, { align: 'right' });
    doc.setFont("THSarabunNew", "normal");
    ...
    if (other === 'none') {
        ...
        checkBox(90, 97, true); // Checkbox for โรคลมชัก
        doc.text(`มี  ${other}`, 93, 98.5); // Add มี text
    }

    doc.text('ลงชื่อ', 80, 110); // Moved up by 2 units
    doc.text(`${newdate[0]}`, 187, 110, { align: 'right' }); // Moved up by 2 units
    ...
    doc.text(`(2) ให้แสดงว่าเป็นผู้มีร่างกายสมบูรณ์เพียงใด ใบรับรองแพทย์ฉบับนี้ใช้ได้ 1 เดือนนับแต่วันที่ตรวจร่างกาย`, 35, 275);
    doc.text(`(3) คำรับรองนี้เป็นการวินิจฉัยเบื้องต้น`, 35, 280);
    doc.text(`แบบฟอร์มนี้ได้รับการรับรองจากมติคณะกรรมการแพทยสภาในการประชุมครั้งที่ 4/2561 วันที่ 19 เมษายน 2561`, 33, 285);
